...
class NoiseComponent extends React.PureComponent {
	...
	prevNoise = () => {
		this.setState({
			currentType: (this.state.currentType - 1 + this.state.noises.length) % this.state.noises.length
		});
		this.noise.type = this.state.noises[this.state.currentType];
	};
	changeVolume = (val) => {
		...
		return (
			<div style={styles.container}>
				...
				<NoiseControls
					...
					octaves={this.state.octaves}
				/>
				<CustomButton classID="NoiseBtn" source={this.noise} click={'Start'} unclick={'Stop'} />
			</div>
		);
	}
}
...
const NoiseSelect = (props) => {
	...
	return (
		<div style={{ textAlign: 'center' }}>
			<ArrowLeftIcon style={styles.icon} onClick={prevNoise} />
			<div style={{ display: 'inline-block', width: '70px' }}>{noises[currentType]}</div>
			<ArrowRightIcon style={styles.icon} onClick={prevNoise} />
		</div>
	);
};
